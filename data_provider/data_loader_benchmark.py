...
class CIDatasetBenchmark(Dataset):
    def __init__(self, root_path='dataset', flag='train', input_len=None, pred_len=None,
        ...
        # print(f'初始化，windows={self.windows}, win_sum={self.win_sum}')
        # init
        assert flag in ['train', 'test', 'val']
        type_map = {'train': 0, 'val': 1, 'test': 2}
        self.set_type = type_map[flag]
    ...
    def __read_data__(self):
        self.scaler = StandardScaler()
        dataset_file_path = self.root_path
        # print('---------------------:{}'.format(dataset_file_path))
        if dataset_file_path.endswith('.csv'):
            df_raw = pd.read_csv(dataset_file_path)
        elif dataset_file_path.endswith('.txt'):
            df_raw = []
        ...
        if self.data_type == 'electricity' or self.data_type == 'weather' or self.data_type == 'traffic':
            data_len = int(len(df_raw)/self.win_sum)
            # print("len(df_raw):{}".format(len(df_raw)))
            # print(self.windows)
            # print('---------')
            num_train = int(data_len * 0.7)
            num_test = int(data_len * 0.2)
            num_vali = data_len - num_train - num_test
            border1s = [data_len*(self.windows-1), data_len*(self.windows-1)+num_train - self.input_len, data_len*(self.windows-1)+data_len - num_test - self.input_len]
            border2s = [data_len*(self.windows-1)+num_train, data_len*(self.windows-1)+num_train + num_vali, data_len*self.windows]

        elif self.data_type == 'ETTm1' or self.data_type == 'ETTh1' or self.data_type == 'ETTh2' or self.data_type == 'ETTm2' :
            data_len = int(len(df_raw)/self.win_sum)
            num_train = int(data_len * 0.6)
            num_test = int(data_len * 0.2)
            num_vali = data_len - num_train - num_test
        ...
        elif self.data_type == 'PEMS03' or self.data_type == 'PEMS04' or self.data_type == 'PEMS07' or self.data_type == 'PEMS08':
            data_len = int(len(df_raw)/self.win_sum)
            num_train = int(data_len * 0.6)
            num_test = int(data_len * 0.2)
            num_vali = data_len - num_train - num_test
...
class CIAutoRegressionDatasetBenchmark(CIDatasetBenchmark):
    def __init__(self, root_path='dataset', flag='train', input_len=None, label_len=None, pred_len=None,
                 data_type='custom', scale=True, timeenc=1, freq='h', stride=1, subset_rand_ratio=1.0, windows=None, win_sum=None):
        ...
        super().__init__(root_path=root_path, flag=flag, input_len=input_len, pred_len=pred_len,
                         ...
                         subset_rand_ratio=subset_rand_ratio, windows=windows, win_sum=win_sum)
