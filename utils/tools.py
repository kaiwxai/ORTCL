...
#     ax = plt.gca()
#     im = ax.imshow(attn, cmap='YlGnBu', aspect='auto')
...
#                 continue
#             val = attn[i, j]
#             color = 'white' if val > (attn.max() * 0.9) else 'black'
...
def attn_map(attn, path='./pic/attn_map.pdf'):
    """
    Attention map visualization with square grids
    """
    plt.figure(figsize=(8, 6))
    ax = plt.gca()
    
    # 将 aspect 设置为 'equal'，保证每个格子是正方形
    im = ax.imshow(attn, cmap='YlGnBu', aspect='equal')
    ...
    # 设置次刻度在格子边界处（-0.5, 0.5, 1.5 ...）
    ax.set_xticks(np.arange(-0.5, attn.shape[1], 1), minor=True)
    ax.set_yticks(np.arange(-0.5, attn.shape[0], 1), minor=True)
    
    # 绘制次刻度网格线，即格子边界线，颜色和宽度可调
    ax.grid(which='minor', color='black', linestyle='-', linewidth=0.05, alpha=0.2)
    ...
    # 调整坐标轴上下左右边框，让网格线完整显示
    ax.set_xlim(-0.5, attn.shape[1] - 0.5)
    ax.set_ylim(attn.shape[0] - 0.5, -0.5)  # y轴反转，保持矩阵第一行在上
